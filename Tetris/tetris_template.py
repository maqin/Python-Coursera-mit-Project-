...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        return Tetris.SHAPES[n](Point((self.BOARD_WIDTH)/2,0))
    ...
    def do_move(self, direction):
        ...
        #YOUR CODE HERE
        p = Point(self.DIRECTION[direction][0],self.DIRECTION[direction][1])
        if self.current_shape.can_move(self.board,p.x,p.y):
            self.current_shape.move(p.x,p.y)
            return True
        if direction == 'Down':
            #self.board[self.current_shape.center_block] = self.current_shape
